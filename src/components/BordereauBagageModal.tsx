...
import { bordereauApi, BordereauBagageData } from '../services/api';
import { ThermalPrinter } from '../services/printer';
import { useAuth } from '../contexts/AuthContext';
import { useToast } from '../contexts/ToastContext';

interface BordereauBagageModalProps {
  isOpen: boolean;
  onClose: () => void;
  departId: number;
}

const BordereauBagageModal: React.FC<BordereauBagageModalProps> = ({
  isOpen,
  onClose,
  departId,
}) => {
  ...
  const [bordereauData, setBordereauData] = useState<BordereauBagageData | null>(null);
  ...
  const loadBordereauData = async () => {
    ...
    try {
      ...
      const response = await bordereauApi.getBordBagage(
        parseInt(user.id),
        agenceId,
        departId
      );
      ...
      console.error('Erreur lors du chargement du bordereau bagage:', error);
      showToast('error', 'Erreur', error.message || 'Impossible de charger le bordereau bagage');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const bagageItems = bordereauData.bagage.map(bagage => ({
        label: `${bagage.exp_code} - ${bagage.exp_coldesc}`,
        value: `${bagage.exp_frais} FCFA`,
      }));

      const ticketData = {
        title: 'BORDEREAU BAGAGE',
        items: [
          ...
          { label: 'Nombre de bagages', value: `${bordereauData.cptbag}` },
          { label: '', value: '' },
          { label: '--- BAGAGES ---', value: '' },
          ...bagageItems,
        ],
      ...
      showToast('success', 'Succès', 'Bordereau bagage imprimé avec succès');
      onClose();
    } catch (error: any) {
            ...
            <div className="space-y-6">
              ...
              <div className="card">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Nombre de bagages</p>
                    <p className="font-medium text-gray-900 dark:text-white">
                      {bordereauData.cptbag}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="card">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                  Liste des bagages ({bordereauData.bagage.length})
                </h3>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {bordereauData.bagage.map((bagage, index) => (
                        <tr
                          key={index}
...
export default BordereauBagageModal;
